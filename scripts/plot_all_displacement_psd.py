...
def compute_psd(file_path, nperseg=131072):
    ...
    omega = 2 * np.pi * f
    disp_psd = np.full_like(pxx, np.nan)

    valid = omega > 0
    disp_psd[valid] = pxx[valid] / omega[valid] ** 4

    return meta, f, pxx, disp_psd
...
def plot_group(files, title, output_name, xlim=(0.05, 0.50)):
    ...
    for file_name in files:
        ...
        meta, f, pxx, disp_psd = compute_psd(file_path)

        label = meta["channel"]
        plt.semilogy(f, disp_psd, label=label)
    ...
    plt.ylabel("Displacement-like PSD [m²/Hz]")
    plt.title(title)
...
def main():
    ...
    plot_group(
        x_files,
        "Displacement-like PSD with ETABS modal frequencies - X direction",
        "disp_psd_x_direction.png",
    )

    plot_group(
        y_files,
        "Displacement-like PSD with ETABS modal frequencies - Y direction",
        "disp_psd_y_direction.png",
    )

    plot_group(
        torsion_files,
        "Displacement-like PSD with ETABS modal frequencies - S3X / torsion-related channels",
        "disp_psd_torsion_channels.png",
    )
